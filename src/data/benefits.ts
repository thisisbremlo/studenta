export type Category =
  | "development"
  | "ai"
  | "security"
  | "productivity"
  | "design"
  | "entertainment"
  | "cloud"
  | "learning"
  | "shopping"
  | "travel"
  | "food"
  | "finance"
  | "mobility"
  | "insurance"
  | "fitness"
  | "culture"
  | "hardware";

export type Pricing = "free" | "freemium" | "paid";

export interface Benefit {
  id: string;
  name: string;
  category: Category;
  pricing: Pricing;
  tagline: string;
  description: string;
  offer: string;
  url: string;
  value: string;
  featured?: boolean;
}

export const categories: { id: Category; label: string; labelDe: string; icon: string }[] = [
  { id: "development", label: "Development", labelDe: "Entwicklung", icon: "Code2" },
  { id: "ai", label: "AI & Coding", labelDe: "KI & Coding", icon: "Sparkles" },
  { id: "security", label: "Security", labelDe: "Sicherheit", icon: "Lock" },
  { id: "productivity", label: "Productivity", labelDe: "Produktivität", icon: "LayoutGrid" },
  { id: "design", label: "Design", labelDe: "Design", icon: "PenTool" },
  { id: "entertainment", label: "Entertainment", labelDe: "Streaming", icon: "Headphones" },
  { id: "cloud", label: "Cloud", labelDe: "Cloud", icon: "Cloud" },
  { id: "learning", label: "Learning", labelDe: "Lernen", icon: "GraduationCap" },
  { id: "shopping", label: "Shopping", labelDe: "Shopping", icon: "ShoppingBag" },
  { id: "travel", label: "Travel", labelDe: "Reisen", icon: "Plane" },
  { id: "food", label: "Food", labelDe: "Essen", icon: "UtensilsCrossed" },
  { id: "finance", label: "Banking", labelDe: "Banking", icon: "Landmark" },
  { id: "mobility", label: "Mobility", labelDe: "Mobilität", icon: "TrainFront" },
  { id: "insurance", label: "Insurance", labelDe: "Versicherungen", icon: "ShieldCheck" },
  { id: "fitness", label: "Fitness", labelDe: "Fitness", icon: "Dumbbell" },
  { id: "culture", label: "Culture", labelDe: "Kultur", icon: "Theater" },
  { id: "hardware", label: "Hardware", labelDe: "Hardware", icon: "Laptop" },
];

export const benefits: Benefit[] = [
  {
    id: "github",
    name: "GitHub Student Developer Pack",
    category: "development",
    pricing: "free",
    tagline: "Pro tools, free for students",
    description:
      "Free GitHub Pro with Copilot Pro, JetBrains IDEs, free domains, cloud credits, and 100+ partner offers. The single most valuable student pack for developers.",
    offer: "Free while enrolled",
    url: "https://education.github.com/pack",
    value: "$200k+ in partner offers",
    featured: true,
  },
  {
    id: "jetbrains",
    name: "JetBrains All Products Pack",
    category: "development",
    pricing: "free",
    tagline: "Every JetBrains IDE, free",
    description:
      "IntelliJ IDEA Ultimate, PyCharm Professional, WebStorm, CLion, Rider, GoLand, DataGrip, and more — all free with a student license. Renew yearly.",
    offer: "Free while enrolled",
    url: "https://www.jetbrains.com/community/education/",
    value: "$249/year per IDE",
    featured: true,
  },
  {
    id: "copilot",
    name: "GitHub Copilot Pro",
    category: "development",
    pricing: "free",
    tagline: "AI pair programming",
    description:
      "AI-powered code completion in VS Code, JetBrains, and Neovim. Included with the GitHub Student Developer Pack at no cost.",
    offer: "Free while enrolled",
    url: "https://github.com/features/copilot",
    value: "$10/month",
  },
  {
    id: "cursor",
    name: "Cursor",
    category: "development",
    pricing: "freemium",
    tagline: "AI-first code editor",
    description:
      "The AI code editor built on VS Code. The legacy free one-year Pro offer for students closed to new sign-ups on June 25, 2026 — the free Hobby plan remains, and previously redeemed offers stay valid until expiry.",
    offer: "Hobby free (Pro offer closed)",
    url: "https://cursor.com/students",
    value: "$20/month (Pro)",
  },
  {
    id: "gemini",
    name: "Google Gemini",
    category: "ai",
    pricing: "freemium",
    tagline: "AI assistant with free tier",
    description:
      "Google's AI assistant with the free tier covering everyday study use. Student pricing and availability vary by country and institution — check the current eligibility in your Google account.",
    offer: "Free tier (student offers vary)",
    url: "https://gemini.google.com/",
    value: "$20/month (Pro)",
  },
  {
    id: "mongodb",
    name: "MongoDB Atlas",
    category: "cloud",
    pricing: "free",
    tagline: "$50 Atlas credits + free certification",
    description:
      "NoSQL document database. Verified students get $50 in Atlas credits, MongoDB Compass, and free MongoDB University certification — about $200 in total value via the GitHub Student Pack.",
    offer: "$50 credits + free certification",
    url: "https://www.mongodb.com/students",
    value: "~$200 value",
  },
  {
    id: "awseducate",
    name: "AWS Educate",
    category: "cloud",
    pricing: "freemium",
    tagline: "AWS labs and credits for learners",
    description:
      "Free learning content, labs, and AWS credits or free-tier access depending on eligibility. Not unlimited free AWS usage — charges can apply beyond the included credits.",
    offer: "Free learning + AWS credits",
    url: "https://aws.amazon.com/education/awseducate/",
    value: "Varies by program",
  },
  {
    id: "gcpedu",
    name: "Google Cloud for Students",
    category: "cloud",
    pricing: "freemium",
    tagline: "Student credits for Google Cloud",
    description:
      "Google Cloud education programs provide student credits, for example Google Skills credits that expire after one year. Amounts and expiry vary by program and region.",
    offer: "Student credits (vary)",
    url: "https://cloud.google.com/edu/students",
    value: "Varies by program",
  },
  {
    id: "oraclecloud",
    name: "Oracle Cloud Free Tier",
    category: "cloud",
    pricing: "free",
    tagline: "Always-free cloud resources",
    description:
      "Oracle Cloud's Free Tier includes always-free resources: two AMD compute VMs, Arm instances with up to 4 OCPUs and 24 GB RAM, plus databases and storage within limits.",
    offer: "Always Free tier",
    url: "https://www.oracle.com/cloud/free/",
    value: "$0 with limits",
  },
  {
    id: "postman",
    name: "Postman Student Expert",
    category: "development",
    pricing: "free",
    tagline: "API platform + free certification",
    description:
      "The industry-standard API platform for building, testing, and documenting APIs. The Student Expert program and certification are free where offered.",
    offer: "Free program + certification",
    url: "https://www.postman.com/student/",
    value: "$0",
  },
  {
    id: "paypalsandbox",
    name: "PayPal Developer Sandbox",
    category: "development",
    pricing: "free",
    tagline: "Free payment testing environment",
    description:
      "Test checkout and payment flows in PayPal's sandbox with fictional accounts and funds — sandbox money is not real, and the developer environment is free.",
    offer: "Free sandbox",
    url: "https://developer.paypal.com/home/",
    value: "$0",
  },
  {
    id: "redhatdev",
    name: "Red Hat Developer Subscription",
    category: "development",
    pricing: "free",
    tagline: "Free RHEL developer subscription",
    description:
      "Free individual developer subscription including Red Hat Enterprise Linux with access to developer tooling and updates — free for all developers, not just students.",
    offer: "Free for individuals",
    url: "https://developers.redhat.com/about",
    value: "$0",
  },
  {
    id: "inteldevzone",
    name: "Intel Developer Zone",
    category: "development",
    pricing: "free",
    tagline: "Free Intel developer resources",
    description:
      "Access Intel's developer tools, SDKs, and learning resources for free. Some tools or support may have separate licensing terms.",
    offer: "Free resources",
    url: "https://developer.intel.com/",
    value: "$0",
  },
  {
    id: "interviewbit",
    name: "InterviewBit",
    category: "learning",
    pricing: "freemium",
    tagline: "Free structured interview prep",
    description:
      "Structured programming interview preparation with timed practice tracks covering data structures, algorithms, and system design. Practice content is free; premium features are paid.",
    offer: "Free practice content",
    url: "https://www.interviewbit.com/",
    value: "Free",
  },
  {
    id: "hackerrank",
    name: "HackerRank Certifications",
    category: "learning",
    pricing: "free",
    tagline: "Free skills certifications",
    description:
      "Free skills-verification certifications for Python, SQL, React, problem solving, and more — a quick line for your résumé or LinkedIn profile. Not all preparation content is free.",
    offer: "Free certifications",
    url: "https://www.hackerrank.com/skills-verification",
    value: "$0",
  },
  {
    id: "kaggle",
    name: "Kaggle",
    category: "learning",
    pricing: "free",
    tagline: "Free notebooks, GPU & datasets",
    description:
      "Free browser notebooks with limited GPU/TPU compute, 100k+ public datasets, and competitions — the standard playground for data-science students. Usage quotas apply.",
    offer: "Free (with quotas)",
    url: "https://www.kaggle.com/",
    value: "$0",
  },
  {
    id: "gdsc",
    name: "Google Developer Groups on Campus",
    category: "learning",
    pricing: "free",
    tagline: "Free community events & workshops",
    description:
      "Google's campus communities run free events, workshops, and study jams on cloud, AI, and Android — occasionally with sponsor credits. Perks vary by club and event.",
    offer: "Free membership",
    url: "https://developers.google.com/community/gdsc",
    value: "$0",
  },
  {
    id: "mslearn",
    name: "Microsoft Learn",
    category: "learning",
    pricing: "free",
    tagline: "Free Microsoft learning paths",
    description:
      "Free interactive learning paths and challenges for Azure, .NET, AI, and more — including student-focused challenges and select free certification offers.",
    offer: "Free",
    url: "https://learn.microsoft.com/training/",
    value: "$0",
  },
  {
    id: "ibmskillsbuild",
    name: "IBM SkillsBuild",
    category: "learning",
    pricing: "free",
    tagline: "Free courses with credentials",
    description:
      "Free learning resources and digital credentials in AI, cybersecurity, data, and sustainability. Course availability varies by region.",
    offer: "Free",
    url: "https://skillsbuild.org/",
    value: "$0",
  },
  {
    id: "udemyfree",
    name: "Udemy Free Courses",
    category: "learning",
    pricing: "free",
    tagline: "Hundreds of free courses",
    description:
      "Udemy's free-course section offers hundreds of complete courses on programming, design, and business — published free by their instructors. Not all Udemy courses are free.",
    offer: "Free courses",
    url: "https://www.udemy.com/courses/free/",
    value: "$0",
  },
  {
    id: "mlh",
    name: "Major League Hacking",
    category: "learning",
    pricing: "freemium",
    tagline: "Hackathons with credits & swag",
    description:
      "The official student hackathon league: free to join events with sponsor cloud credits, swag, and prizes. Availability varies by event and sponsor.",
    offer: "Free to participate",
    url: "https://mlh.io/",
    value: "Varies by event",
  },
  {
    id: "unidays",
    name: "UNiDAYS",
    category: "shopping",
    pricing: "free",
    tagline: "Free student discount portal",
    description:
      "Free membership that unlocks brand discounts across fashion, tech, food, and fitness — Nike, adidas, ASOS, McFit, and many more. Discounts vary by brand and country.",
    offer: "Free membership",
    url: "https://www.myunidays.com/",
    value: "Discounts at 100+ brands",
  },
  {
    id: "studentbeans",
    name: "Student Beans",
    category: "shopping",
    pricing: "free",
    tagline: "Free student discount portal",
    description:
      "Free student membership with discount codes across hundreds of brands — an alternative to UNiDAYS with a partly different partner list. Discounts vary by brand and country.",
    offer: "Free membership",
    url: "https://www.studentbeans.com/",
    value: "Discounts at 100+ brands",
  },
  {
    id: "blender",
    name: "Blender",
    category: "design",
    pricing: "free",
    tagline: "Open-source 3D creation suite",
    description:
      "The free and open-source 3D suite for modeling, sculpting, animation, rendering, and VFX — no student verification needed, it's free for everyone.",
    offer: "Free (open source)",
    url: "https://www.blender.org/download/",
    value: "$0",
  },
  {
    id: "miro",
    name: "Miro Education",
    category: "productivity",
    pricing: "free",
    tagline: "Collaborative whiteboard, free",
    description:
      "The infinite collaborative whiteboard for brainstorming, diagrams, and group work. The Education plan is free for eligible students and educators; verification may be renewed.",
    offer: "Free while enrolled",
    url: "https://miro.com/education-whiteboard/",
    value: "$8/month",
  },
  {
    id: "loom",
    name: "Loom Education",
    category: "productivity",
    pricing: "free",
    tagline: "Async video messages, free",
    description:
      "Record quick screen and camera videos to explain code, give feedback, or present projects asynchronously. The Education plan is free for eligible students and educators.",
    offer: "Free while enrolled",
    url: "https://www.loom.com/education",
    value: "$15/month",
  },
  {
    id: "clickup",
    name: "ClickUp",
    category: "productivity",
    pricing: "paid",
    tagline: "All-in-one task & project workspace",
    description:
      "Tasks, docs, goals, and whiteboards in one workspace. Students, educators, and academic institutions get discounts on paid plans — check current eligibility and terms.",
    offer: "Education discount",
    url: "https://clickup.com/education",
    value: "Save on Unlimited/Business",
  },
  {
    id: "airtable",
    name: "Airtable for Students",
    category: "productivity",
    pricing: "free",
    tagline: "Team plan free up to 24 months",
    description:
      "Spreadsheet-database hybrid for organizing anything. Qualifying college and university students get 6–24 months of complimentary Team-plan access based on expected graduation date.",
    offer: "Free 6–24 months",
    url: "https://support.airtable.com/articles/4920135568-applying-for-non-profit-education-and-student-airtable-plans",
    value: "$20/user/month",
  },
  {
    id: "sketch",
    name: "Sketch Education",
    category: "design",
    pricing: "paid",
    tagline: "Mac-native UI design, ~50% off",
    description:
      "Mac-native UI design tool with vector editing and prototyping. Education pricing is roughly half price for eligible students — historically around 50% off rather than free.",
    offer: "~50% off",
    url: "https://www.sketch.com/store/education/",
    value: "Save ~50%",
  },
  {
    id: "blenderswap",
    name: "Sketchfab CC Assets",
    category: "design",
    pricing: "free",
    tagline: "Free CC 3D models for your projects",
    description:
      "Sketchfab hosts thousands of free Creative-Commons-licensed 3D models you can use in Blender, Unity, and game projects — handy assets for design and game-dev coursework.",
    offer: "Free CC models",
    url: "https://sketchfab.com/",
    value: "$0",
  },
  {
    id: "hackthebox",
    name: "Hack The Box Academy",
    category: "security",
    pricing: "paid",
    tagline: "Hands-on hacking labs, student plan",
    description:
      "Hands-on cybersecurity labs and guided modules from beginner to certification level. Verified students get the Academy subscription at $8/month.",
    offer: "$8/month student plan",
    url: "https://academy.hackthebox.com/",
    value: "Save vs. standard tiers",
  },
  {
    id: "tryhackme",
    name: "TryHackMe",
    category: "security",
    pricing: "paid",
    tagline: "Guided hacking labs, student discount",
    description:
      "Beginner-friendly, guided cybersecurity labs with learning paths from first scan to red team. Verified students get a discount on Premium subscriptions.",
    offer: "Student discount",
    url: "https://tryhackme.com/",
    value: "Save vs. Premium",
  },
  {
    id: "perplexity",
    name: "Perplexity Education Pro",
    category: "development",
    pricing: "paid",
    tagline: "AI research assistant, 50% off",
    description:
      "AI-powered answer engine with advanced models. Students and educators get 50% off the Pro plan after SheerID verification — roughly $9/month instead of $20.",
    offer: "50% off (was $20/month)",
    url: "https://www.perplexity.ai/education",
    value: "Save $11/month",
  },
  {
    id: "microsoft365",
    name: "Microsoft 365 Education",
    category: "productivity",
    pricing: "free",
    tagline: "Word, Excel, PowerPoint & 1 TB OneDrive",
    description:
      "Full Office suite including Word, Excel, PowerPoint, OneNote, and Teams — plus 1 TB of OneDrive cloud storage. Free with a valid school email.",
    offer: "Free while enrolled",
    url: "https://www.microsoft.com/education/products/office",
    value: "$99/year",
    featured: true,
  },
  {
    id: "notion",
    name: "Notion Education Plus",
    category: "productivity",
    pricing: "free",
    tagline: "All Plus features, free",
    description:
      "Unlimited blocks, file uploads, version history, and advanced collaboration. Sign up with your school email and switch to the Education plan in billing settings.",
    offer: "Free while enrolled",
    url: "https://www.notion.so/students",
    value: "$10/month",
  },
  {
    id: "figma",
    name: "Figma Education",
    category: "design",
    pricing: "free",
    tagline: "Professional design, free",
    description:
      "Full Figma Professional plan with unlimited projects, unlimited editors, and FigJam collaborative whiteboarding. The industry-standard UI/UX design tool.",
    offer: "Free while enrolled",
    url: "https://www.figma.com/education/",
    value: "$15/month",
    featured: true,
  },
  {
    id: "adobe",
    name: "Adobe Creative Cloud",
    category: "design",
    pricing: "paid",
    tagline: "60%+ off the full suite",
    description:
      "Photoshop, Illustrator, InDesign, Premiere Pro, After Effects, Lightroom, and 20+ more creative apps. Verify with your .edu email through SheerID.",
    offer: "60%+ off",
    url: "https://www.adobe.com/creativecloud/buy/students.html",
    value: "$65.99/month → $19.97/month",
  },
  {
    id: "canva",
    name: "Canva for Education",
    category: "design",
    pricing: "free",
    tagline: "Pro design tools, free",
    description:
      "Canva Pro with 100+ million photos, videos, audio, and graphics. Create presentations, social media, posters, and more — free for students and educators.",
    offer: "Free while enrolled",
    url: "https://www.canva.com/education/",
    value: "$15/month",
  },
  {
    id: "autodesk",
    name: "Autodesk Education",
    category: "design",
    pricing: "free",
    tagline: "AutoCAD, Maya, Fusion 360 & more",
    description:
      "Free 1-year educational licenses for AutoCAD, Fusion 360, Maya, 3ds Max, Revit, Inventor, and the full Autodesk portfolio. Ideal for engineering and 3D design.",
    offer: "Free 1-year license",
    url: "https://www.autodesk.com/education",
    value: "$2,000+/year",
  },
  {
    id: "unity",
    name: "Unity Student",
    category: "design",
    pricing: "free",
    tagline: "Game engine, free",
    description:
      "Free Unity Student plan with all core Unity features for game development and real-time 3D. Includes the same engine used by professionals, no watermark.",
    offer: "Free while enrolled",
    url: "https://unity.com/products/unity-student",
    value: "$2,200/year",
  },
  {
    id: "spotify",
    name: "Spotify Premium Student + Hulu",
    category: "entertainment",
    pricing: "paid",
    tagline: "Music + TV, bundled",
    description:
      "Ad-free Spotify Premium plus Hulu (With Ads) for a single discounted price. First month free, then $6.99/month. Eligible for up to 4 years.",
    offer: "$6.99/month (was $24.98)",
    url: "https://www.spotify.com/student",
    value: "Save $17.99/month",
    featured: true,
  },
  {
    id: "applemusic",
    name: "Apple Music Student + Apple TV+",
    category: "entertainment",
    pricing: "paid",
    tagline: "100M songs + Apple TV+, free",
    description:
      "Ad-free Apple Music with Spatial Audio plus free access to Apple TV+ shows like Severance and The Morning Show. $5.99/month for eligible students.",
    offer: "$5.99/month (was $19.98)",
    url: "https://offers.applemusic.apple/student-offer",
    value: "Save $13.99/month",
  },
  {
    id: "amazon",
    name: "Amazon Prime for Young Adults",
    category: "entertainment",
    pricing: "paid",
    tagline: "6 months free, then 50% off",
    description:
      "Free 2-day shipping, Prime Video, Prime Music, and 5% cash back. Six months free for students, then $7.49/month or $69/year — half the regular price.",
    offer: "6 months free, then $7.49/mo",
    url: "https://www.amazon.com/amazonprime",
    value: "Save $139/year",
  },
  {
    id: "youtube",
    name: "YouTube Premium Student",
    category: "entertainment",
    pricing: "paid",
    tagline: "Ad-free YouTube + Music",
    description:
      "Ad-free videos, background play, offline downloads, and YouTube Music Premium included. Verify student status with SheerID.",
    offer: "$7.99/month (was $13.99)",
    url: "https://www.youtube.com/premium",
    value: "Save $6/month",
  },
  {
    id: "peacock",
    name: "Peacock Premium Student",
    category: "entertainment",
    pricing: "paid",
    tagline: "NBC, movies & live sports",
    description:
      "Stream NBC shows, blockbuster movies, Premier League, and live sports. Students get Peacock Premium for $2.99/month — 73% off the regular price.",
    offer: "$2.99/month (was $10.99)",
    url: "https://www.peacocktv.com/student",
    value: "Save $8/month",
  },
  {
    id: "paramount",
    name: "Paramount+ Student",
    category: "entertainment",
    pricing: "paid",
    tagline: "50% off all plans",
    description:
      "Stream movies, shows, and live sports from Paramount. Students get 50% off any plan for the first 12 months — starting at $4.50/month.",
    offer: "50% off (from $4.50/mo)",
    url: "https://www.paramountplus.com/student",
    value: "Save 50%",
  },
  {
    id: "digitalocean",
    name: "DigitalOcean Cloud Credits",
    category: "cloud",
    pricing: "free",
    tagline: "$200 in cloud hosting",
    description:
      "Spin up droplets, manage Kubernetes, and deploy apps with $200 in DigitalOcean cloud credits. Included in the GitHub Student Developer Pack.",
    offer: "$200 free credits",
    url: "https://education.github.com/pack",
    value: "$200",
  },
  {
    id: "azure",
    name: "Microsoft Azure for Students",
    category: "cloud",
    pricing: "free",
    tagline: "$100 in Azure credits",
    description:
      "Build, deploy, and manage apps on Microsoft Azure with $100 in free credits and access to free-tier services. No credit card required to start.",
    offer: "$100 free credits",
    url: "https://azure.microsoft.com/free/students",
    value: "$100",
  },
  {
    id: "namecheap",
    name: "Namecheap .me Domain + SSL",
    category: "cloud",
    pricing: "free",
    tagline: "Free domain + SSL for 1 year",
    description:
      "Register a free .me domain for one year and get a free SSL certificate. Perfect for building your personal portfolio site. Via the GitHub Student Pack.",
    offer: "Free 1-year domain",
    url: "https://education.github.com/pack",
    value: "$20/year",
  },
  {
    id: "namedotcom",
    name: "Name.com Domain",
    category: "cloud",
    pricing: "free",
    tagline: "Free domain across 25+ extensions",
    description:
      "Get a free domain name for one year from Name.com, available across 25+ extensions. Build your personal brand or project site. Via the GitHub Student Pack.",
    offer: "Free 1-year domain",
    url: "https://education.github.com/pack",
    value: "$15/year",
  },
  {
    id: "heroku",
    name: "Heroku Postgres + Dynos",
    category: "cloud",
    pricing: "free",
    tagline: "Free app hosting tier",
    description:
      "Deploy web apps with Heroku's free dynos and a free Heroku Postgres database. Great for hosting side projects and portfolios. Via the GitHub Student Pack.",
    offer: "Free tier access",
    url: "https://education.github.com/pack",
    value: "$5/month",
  },
  {
    id: "frontendmasters",
    name: "Frontend Masters",
    category: "learning",
    pricing: "free",
    tagline: "Expert-led courses, free",
    description:
      "Access to the full Frontend Masters library of expert-led courses on JavaScript, React, CSS, and more. Free subscription via the GitHub Student Pack.",
    offer: "Free 6-month subscription",
    url: "https://education.github.com/pack",
    value: "$39/month",
  },
  {
    id: "datacamp",
    name: "DataCamp",
    category: "learning",
    pricing: "free",
    tagline: "Data science courses, free",
    description:
      "Learn Python, R, SQL, and machine learning with interactive courses and projects. Free 3-month subscription through the GitHub Student Pack.",
    offer: "Free 3-month subscription",
    url: "https://education.github.com/pack",
    value: "$39/month",
  },
  {
    id: "codecademy",
    name: "Codecademy Pro",
    category: "learning",
    pricing: "paid",
    tagline: "35% off Pro",
    description:
      "Interactive coding courses in Python, JavaScript, Java, C++, SQL, and more. Get 35% off the Pro subscription with student verification.",
    offer: "35% off Pro",
    url: "https://www.codecademy.com/pages/student-discount",
    value: "Save ~$140/year",
  },
  {
    id: "nytimes",
    name: "The New York Times",
    category: "learning",
    pricing: "paid",
    tagline: "Digital access for $1/week",
    description:
      "Unlimited access to NYT articles, cooking, games, and The Athletic. Special introductory rate of $1/week for students for the first year.",
    offer: "$1/week for 1 year",
    url: "https://www.nytimes.com/subscription/student",
    value: "Save 80%+",
  },
  {
    id: "grammarly",
    name: "Grammarly Premium",
    category: "productivity",
    pricing: "freemium",
    tagline: "AI writing assistant",
    description:
      "Grammar, spelling, and style suggestions everywhere you write. The core version is free for students; Premium unlocks advanced suggestions.",
    offer: "Free basic plan",
    url: "https://www.grammarly.com",
    value: "€12/month (Premium)",
  },
  {
    id: "zotero",
    name: "Zotero",
    category: "productivity",
    pricing: "free",
    tagline: "Reference manager, free for everyone",
    description:
      "Collect, organize, cite, and share your research sources. The open-source reference manager every thesis writer needs — completely free.",
    offer: "Free",
    url: "https://www.zotero.org",
    value: "Priceless for your thesis",
  },
  {
    id: "anki",
    name: "Anki",
    category: "learning",
    pricing: "free",
    tagline: "Spaced-repetition flashcards",
    description:
      "The memorization powerhouse used by medical students worldwide. Desktop and Android versions are free — only the iOS app costs money.",
    offer: "Free (Desktop/Android)",
    url: "https://apps.ankiweb.net",
    value: "€27.99 (iOS)",
  },
  {
    id: "obsidian",
    name: "Obsidian",
    category: "productivity",
    pricing: "freemium",
    tagline: "Markdown notes on a knowledge graph",
    description:
      "A powerful note-taking app built on local Markdown files. Free for personal use including all core plugins; Sync and Publish are paid.",
    offer: "Free core app",
    url: "https://obsidian.md",
    value: "$2.4/month (Sync)",
  },
  {
    id: "appletvplus",
    name: "Apple TV+ with Apple Music Student",
    category: "entertainment",
    pricing: "free",
    tagline: "Apple TV+ included at no cost",
    description:
      "Subscribe to Apple Music's student plan and get Apple TV+ included for free — Severance, Ted Lasso, and more at no extra charge.",
    offer: "Included free",
    url: "https://www.apple.com/de/apple-tv-plus/",
    value: "€6.99/month saved",
  },
  {
    id: "quizlet",
    name: "Quizlet Plus",
    category: "learning",
    pricing: "freemium",
    tagline: "Flashcards & study modes",
    description:
      "Create flashcards and study with learn, test, and game modes. The basic version is free; Plus unlocks advanced creation and offline access.",
    offer: "Free basic plan",
    url: "https://quizlet.com",
    value: "€7.99/month (Plus)",
  },
  {
    id: "studysmarter",
    name: "StudySmarter",
    category: "learning",
    pricing: "freemium",
    tagline: "Study app made in Germany",
    description:
      "Shared flashcards, study groups, and summaries synced across devices. Free basic plan for students, Vaia Plus is the paid tier.",
    offer: "Free basic plan",
    url: "https://studysmarter.de",
    value: "€9.99/month (Plus)",
  },
  {
    id: "khanacademy",
    name: "Khan Academy",
    category: "learning",
    pricing: "free",
    tagline: "Free courses on everything",
    description:
      "World-class lessons in math, science, economics, and more with practice exercises and progress tracking. Nonprofit and 100% free forever.",
    offer: "Free",
    url: "https://de.khanacademy.org",
    value: "Free forever",
  },
  {
    id: "duolingo",
    name: "Duolingo Super",
    category: "learning",
    pricing: "freemium",
    tagline: "Language learning, gamified",
    description:
      "Learn 40+ languages with bite-sized daily lessons. The free tier covers everything; Super removes ads and adds unlimited hearts.",
    offer: "Free basic plan",
    url: "https://www.duolingo.com",
    value: "€12.99/month (Super)",
  },
  {
    id: "blinkist",
    name: "Blinkist",
    category: "learning",
    pricing: "paid",
    tagline: "Book summaries in 15 minutes",
    description:
      "Key insights from thousands of nonfiction books and podcasts as 15-minute reads or audio. Student discount brings it to about €6.67/month.",
    offer: "~55% off (€6.67/month)",
    url: "https://www.blinkist.com/de",
    value: "Save €8.32/month",
  },
  {
    id: "coursera",
    name: "Coursera",
    category: "learning",
    pricing: "freemium",
    tagline: "University courses, free to audit",
    description:
      "Courses from Stanford, Google, IBM, and hundreds of universities. Audit nearly any course for free; certificates and graded work need Plus.",
    offer: "Free (Audit mode)",
    url: "https://www.coursera.org",
    value: "$59/month (Plus)",
  },
  {
    id: "courserafinancialaid",
    name: "Coursera Financial Aid",
    category: "learning",
    pricing: "free",
    tagline: "Courses free via financial aid",
    description:
      "Coursera's financial aid can make an individual course — including its graded certificate — free after approval. Applications are reviewed per course and take about two weeks.",
    offer: "Free (per-course application)",
    url: "https://www.coursera.org/financial-aid",
    value: "$49–99/course saved",
  },
  {
    id: "edx",
    name: "edX",
    category: "learning",
    pricing: "free",
    tagline: "MIT & Harvard courses, free to audit",
    description:
      "Access courses from MIT, Harvard, and top universities worldwide in audit mode at no cost — a goldmine for computer science and engineering students.",
    offer: "Free (Audit mode)",
    url: "https://www.edx.org",
    value: "Free",
  },
  {
    id: "asos",
    name: "ASOS Student",
    category: "shopping",
    pricing: "paid",
    tagline: "10% off everything",
    description:
      "10% off fashion orders at ASOS after student verification via UNiDAYS — stacks on top of existing sale prices.",
    offer: "10% off",
    url: "https://www.asos.com/de/",
    value: "Save 10%",
  },
  {
    id: "aboutyou",
    name: "ABOUT YOU Student",
    category: "shopping",
    pricing: "paid",
    tagline: "10% off fashion",
    description:
      "Verified students get 10% off their orders at ABOUT YOU, one of Europe's biggest online fashion retailers.",
    offer: "10% off",
    url: "https://www.aboutyou.de",
    value: "Save 10%",
  },
  {
    id: "nike",
    name: "Nike Student",
    category: "shopping",
    pricing: "paid",
    tagline: "20% off sneakers & apparel",
    description:
      "One of the best student fashion discounts out there: 20% off at Nike.com after verifying with UNiDAYS or SheerID.",
    offer: "20% off",
    url: "https://www.nike.com/de/",
    value: "Save 20%",
  },
  {
    id: "adidas",
    name: "adidas Student",
    category: "shopping",
    pricing: "paid",
    tagline: "15% off sportswear",
    description:
      "15% off full-price items on adidas.com for verified students — often combinable with ongoing sales.",
    offer: "15% off",
    url: "https://www.adidas.de/",
    value: "Save 15%",
  },
  {
    id: "zalando",
    name: "Zalando via UNiDAYS",
    category: "shopping",
    pricing: "paid",
    tagline: "10% off via UNiDAYS",
    description:
      "Europe's biggest fashion platform offers students 10% off through UNiDAYS verification — applies during periodic student promotions.",
    offer: "10% off",
    url: "https://www.myunidays.com/DE/de-DE",
    value: "Save 10%",
  },
  {
    id: "samsung",
    name: "Samsung Education Store",
    category: "shopping",
    pricing: "paid",
    tagline: "Up to 15% off phones & laptops",
    description:
      "Student pricing on Galaxy phones, tablets, laptops, and monitors — up to 15% off plus extra promotions on top.",
    offer: "Up to 15% off",
    url: "https://www.samsung.com/de/",
    value: "Save up to 15%",
  },
  {
    id: "hostelworld",
    name: "Hostelworld Student",
    category: "travel",
    pricing: "paid",
    tagline: "10% off hostel bookings",
    description:
      "10% off hostel stays worldwide for students — perfect for semester breaks and interrail trips on a budget.",
    offer: "10% off",
    url: "https://www.hostelworld.com",
    value: "Save 10%",
  },
  {
    id: "flixtrain",
    name: "FlixTrain Student",
    category: "travel",
    pricing: "paid",
    tagline: "15% off train tickets",
    description:
      "Students save 15% on FlixTrain long-distance train tickets across Germany — cheap weekend trips home.",
    offer: "15% off",
    url: "https://www.flixtrain.de",
    value: "Save 15%",
  },
  {
    id: "hotelscom",
    name: "Hotels.com Student",
    category: "travel",
    pricing: "paid",
    tagline: "10% off hotel bookings",
    description:
      "10% off select hotel bookings for verified students — book your travel for field trips and vacations for less.",
    offer: "10% off",
    url: "https://www.hotels.com",
    value: "Save 10%",
  },
  {
    id: "fluege",
    name: "fluege.de Student",
    category: "travel",
    pricing: "paid",
    tagline: "€10 off flight bookings",
    description:
      "Get a one-time €10 discount on flight bookings as a student — handy for semester-abroad travel and gap-year trips.",
    offer: "€10 off",
    url: "https://www.fluege.de",
    value: "Save €10",
  },
  {
    id: "emirates",
    name: "Emirates Student",
    category: "travel",
    pricing: "paid",
    tagline: "10% off long-haul flights",
    description:
      "Students fly 10% cheaper with Emirates, including extra baggage allowance on eligible fares — great for study abroad and travel home.",
    offer: "10% off",
    url: "https://www.emirates.com/de/",
    value: "Save 10%",
  },
  {
    id: "lieferando",
    name: "Lieferando Student",
    category: "food",
    pricing: "paid",
    tagline: "€12–13 one-time discount",
    description:
      "One-time student discount of €12–13 off your Lieferando order after verification — a free dinner on them.",
    offer: "€12–13 off (one-time)",
    url: "https://www.lieferando.de",
    value: "Save ~€12",
  },
  {
    id: "mcdonalds",
    name: "McDonald's via Lieferando",
    category: "food",
    pricing: "paid",
    tagline: "€10 off delivery",
    description:
      "Order McDonald's through Lieferando as a student and get a one-time €10 discount on your delivery.",
    offer: "€10 off (one-time)",
    url: "https://www.lieferando.de",
    value: "Save €10",
  },
  {
    id: "ing",
    name: "ING Girokonto",
    category: "finance",
    pricing: "free",
    tagline: "Free checking account for students",
    description:
      "Germany's popular direct bank waives all account management fees for students — free card, free payments, no minimum income required.",
    offer: "Free while enrolled",
    url: "https://www.ing.de",
    value: "€4.90/month saved",
  },
  {
    id: "sparkasse",
    name: "Sparkasse Girokonto",
    category: "finance",
    pricing: "free",
    tagline: "Free student account at your local branch",
    description:
      "Most local Sparkasse branches offer students a free checking account with debit card — useful if you want in-person banking.",
    offer: "Free while enrolled",
    url: "https://www.sparkasse.de",
    value: "€3–5/month saved",
  },
  {
    id: "comdirect",
    name: "Comdirect Girokonto",
    category: "finance",
    pricing: "free",
    tagline: "Free online checking account",
    description:
      "Commerzbank's direct-banking brand gives students a free current account including a Visa debit card.",
    offer: "Free while enrolled",
    url: "https://www.comdirect.de",
    value: "€4.90/month saved",
  },
  {
    id: "sparda",
    name: "Sparda-Bank Girokonto",
    category: "finance",
    pricing: "free",
    tagline: "Free account, no age limit tricks",
    description:
      "Students bank for free at Sparda-Bank — no account management fees while enrolled, with debit card and online banking included.",
    offer: "Free while enrolled",
    url: "https://www.sparda.de",
    value: "€3–5/month saved",
  },
  {
    id: "bahncard",
    name: "Deutsche Bahn BahnCard 50",
    category: "mobility",
    pricing: "paid",
    tagline: "50% off every train ride",
    description:
      "Students pay only about €60/year instead of €127 for the BahnCard 50 — half price on all DB journeys, pays for itself in a few trips.",
    offer: "~€60/year (was €127)",
    url: "https://www.bahn.de",
    value: "Save ~53%",
    featured: true,
  },
  {
    id: "sixt",
    name: "Sixt Student",
    category: "mobility",
    pricing: "paid",
    tagline: "Special rental car rates",
    description:
      "Students get special rates on Sixt rental cars — useful for moving apartments or weekend getaways when trains don't cut it.",
    offer: "Special student rates",
    url: "https://www.sixt.de",
    value: "Variable savings",
  },
  {
    id: "starcar",
    name: "Starcar Student",
    category: "mobility",
    pricing: "paid",
    tagline: "Discounted car & transporter rentals",
    description:
      "Student special rates on rental cars and transporters at Starcar — the budget option for moving day.",
    offer: "Special student rates",
    url: "https://www.starcar.de",
    value: "Variable savings",
  },
  {
    id: "mendeley",
    name: "Mendeley",
    category: "productivity",
    pricing: "free",
    tagline: "Reference manager by Elsevier",
    description:
      "Free reference manager and academic social network. Organize PDFs, annotate, and auto-generate citations in Word or LaTeX.",
    offer: "Free",
    url: "https://www.mendeley.com",
    value: "Free reference management",
  },
  {
    id: "citavi",
    name: "Citavi",
    category: "productivity",
    pricing: "free",
    tagline: "Reference & knowledge management",
    description:
      "Popular German reference manager for literature research and citations. Free for students at many universities via campus licenses.",
    offer: "Free (Campus license)",
    url: "https://www.citavi.com",
    value: "€119/year (regular)",
  },
  {
    id: "netflix",
    name: "Netflix",
    category: "entertainment",
    pricing: "paid",
    tagline: "Streaming — no student discount",
    description:
      "The biggest streaming service has no student pricing. Listed here so you know: don't expect a discount, and maybe share a plan instead.",
    offer: "No student discount",
    url: "https://www.netflix.com/de/",
    value: "From €7.99/month",
  },
  {
    id: "disneyplus",
    name: "Disney+",
    category: "entertainment",
    pricing: "paid",
    tagline: "Streaming — no student discount",
    description:
      "Disney, Marvel, Star Wars, and Pixar — but no student pricing in Germany. Bundle options with other services may save you more.",
    offer: "No student discount",
    url: "https://www.disneyplus.com/de",
    value: "From €8.99/month",
  },
  {
    id: "audible",
    name: "Audible Student",
    category: "entertainment",
    pricing: "paid",
    tagline: "50% off audiobooks",
    description:
      "One audiobook credit per month plus exclusive originals. Students pay €4.99/month instead of €9.99 — half price while enrolled.",
    offer: "€4.99/month (was €9.99)",
    url: "https://www.audible.de",
    value: "Save 50%",
  },
  {
    id: "skyticket",
    name: "Sky Ticket Student",
    category: "entertainment",
    pricing: "paid",
    tagline: "50% off sports & shows",
    description:
      "Bundesliga, F1, and Sky original series streamed live. Students pay €14.99/month instead of €29.99.",
    offer: "€14.99/month (was €29.99)",
    url: "https://www.sky.de",
    value: "Save 50%",
  },
  {
    id: "jstor",
    name: "JSTOR Personal Account",
    category: "learning",
    pricing: "free",
    tagline: "Academic journals, free tier",
    description:
      "Registered personal accounts can read 10 articles per 30 days free from JSTOR's scholarly archive. Access through your university library is usually far more generous — check there first for term papers.",
    offer: "10 articles/30 days free",
    url: "https://about.jstor.org/students-and-researchers/",
    value: "$19.50/month (regular)",
  },
  {
    id: "googleaiplus",
    name: "Google AI Plus for Students",
    category: "learning",
    pricing: "free",
    tagline: "Gemini Pro free for 1 year",
    description:
      "Google gives students a free year of its AI Plus plan: Gemini advanced models, NotebookLM Plus, and 2 TB storage for studying.",
    offer: "Free 1-year subscription",
    url: "https://blog.google/intl/de-de/produkte/suchen-entdecken/gemini-lernfunktionen-kostenloses-abo/",
    value: "$20/month value",
  },
  {
    id: "isic",
    name: "ISIC Card",
    category: "mobility",
    pricing: "paid",
    tagline: "The international student ID",
    description:
      "The UNESCO-endorsed international student identity card unlocks tens of thousands of discounts in 130+ countries worldwide.",
    offer: "€15/year",
    url: "https://www.isic.de",
    value: "Discounts in 130+ countries",
  },
  {
    id: "appleedu",
    name: "Apple Education Store",
    category: "shopping",
    pricing: "paid",
    tagline: "5–15% off + gift card",
    description:
      "Students get 5–9% off Macs and iPads, often plus a gift card of up to €120 during back-to-school promos — directly in Apple's education store.",
    offer: "5–15% off",
    url: "https://www.apple.com/de-edu/shop",
    value: "Save up to €120 extra",
  },
  {
    id: "ikea",
    name: "IKEA Student (Back-to-College)",
    category: "shopping",
    pricing: "paid",
    tagline: "15% off, seasonal",
    description:
      "During IKEA's seasonal back-to-college campaigns students save 15% on furnishings — perfect for setting up your first dorm or WG room.",
    offer: "15% off (seasonal)",
    url: "https://www.ikea.com",
    value: "Save 15%",
  },
  {
    id: "home24",
    name: "home24 Student",
    category: "shopping",
    pricing: "paid",
    tagline: "11% off furniture & decor",
    description:
      "11% student discount on furniture, lamps, and home accessories at home24 — build your study den for less.",
    offer: "11% off",
    url: "https://www.home24.de",
    value: "Save 11%",
  },
  {
    id: "emma",
    name: "Emma Matratze Student",
    category: "shopping",
    pricing: "paid",
    tagline: "15% off mattresses",
    description:
      "15% off Emma mattresses and bedding for students — because good sleep is the cheapest study performance booster.",
    offer: "15% off",
    url: "https://www.emma-matratze.de",
    value: "Save 15%",
  },
  {
    id: "studentuniverse",
    name: "StudentUniverse",
    category: "travel",
    pricing: "paid",
    tagline: "Up to 20% off flights",
    description:
      "Flight bookings for verified students with fares up to 20% below regular prices — built specifically for student travel.",
    offer: "Up to 20% off",
    url: "https://www.studentuniverse.de",
    value: "Save up to 20%",
  },
  {
    id: "foodora",
    name: "Foodora Student",
    category: "food",
    pricing: "paid",
    tagline: "€5–10 off food delivery",
    description:
      "Foodora's pink riders deliver with periodic student discounts of €5–10 on your order — another reason to skip cooking.",
    offer: "€5–10 off",
    url: "https://www.foodora.de",
    value: "Save €5–10",
  },
  {
    id: "dkb",
    name: "DKB Girokonto",
    category: "finance",
    pricing: "free",
    tagline: "Free account + free card worldwide",
    description:
      "Germany's largest direct bank gives students a free checking account with a Visa debit card that's free to use at ATMs worldwide.",
    offer: "Free while enrolled",
    url: "https://www.dkb.de",
    value: "€4.90/month saved",
  },
  {
    id: "hanseatic",
    name: "Hanseatic Credit Card",
    category: "finance",
    pricing: "free",
    tagline: "Fee-free student credit card",
    description:
      "Student versions of credit cards like Hanseatic Bank's waive the usual €20–40 annual fee — build a credit history for free.",
    offer: "Free annual fee",
    url: "https://www.hanseatic-bank.de",
    value: "€20–40/year saved",
  },
  {
    id: "studentsid",
    name: "ISIC Airline Student Fares",
    category: "travel",
    pricing: "paid",
    tagline: "Student fares on airlines",
    description:
      "Student fares and discounts on flights vary by airline and booking platform — many carriers offer them, but there is no single official global program. Check each airline's student fare policy.",
    offer: "Varies by airline",
    url: "https://www.iata.org/",
    value: "Check per airline",
  },
  {
    id: "kv-u25",
    name: "Health Insurance (under 25)",
    category: "insurance",
    pricing: "free",
    tagline: "Family coverage — €0 per month",
    description:
      "If you're under 25 and studying, you're covered free through family insurance with statutory health insurers like TK, AOK, or Barmer.",
    offer: "Free (family coverage)",
    url: "https://www.studysmarter.de/s/versicherung/krankenkasse/studenten/",
    value: "~€140/month saved",
    featured: true,
  },
  {
    id: "kv-25-30",
    name: "Health Insurance (25–30)",
    category: "insurance",
    pricing: "paid",
    tagline: "Student rate ~€130–140/month",
    description:
      "Students over 25 pay the discounted student statutory rate of roughly €130–140/month instead of the full ~€230 voluntary rate.",
    offer: "~€130–140/month",
    url: "https://www.studysmarter.de/s/versicherung/krankenkasse/studenten/",
    value: "Save ~40%",
  },
  {
    id: "haftpflicht",
    name: "Liability Insurance (Young Tariff)",
    category: "insurance",
    pricing: "paid",
    tagline: "Private liability from ~€30/year",
    description:
      "Personal liability insurance is the one policy everyone should have. Young/student tariffs from insurers like HDI cost about €30–35/year.",
    offer: "~€30–35/year",
    url: "https://www.hdi.de",
    value: "Save ~30%",
  },
  {
    id: "mcfit",
    name: "McFit Student",
    category: "fitness",
    pricing: "paid",
    tagline: "~17% off gym membership",
    description:
      "Germany's biggest gym chain offers students a discounted membership of about €24.99/month instead of €29.99 — via UNiDAYS.",
    offer: "~€24.99/month",
    url: "https://www.myunidays.com/DE/de-DE/partners/mcfit/view",
    value: "Save ~17%",
  },
  {
    id: "allinclusive",
    name: "All Inclusive Fitness",
    category: "fitness",
    pricing: "paid",
    tagline: "40% off gym membership",
    description:
      "Multi-gym access across Germany with student pricing around €17.99/month instead of €29.99 — via UNiDAYS.",
    offer: "€17.99/month (was €29.99)",
    url: "https://www.myunidays.com/DE/de-DE",
    value: "Save 40%",
  },
  {
    id: "hochschulsport",
    name: "University Sports (Hochschulsport)",
    category: "fitness",
    pricing: "free",
    tagline: "Cheap courses from your university",
    description:
      "Yoga, climbing, football, rowing — university sports programs cost between €0 and €30 per semester, a fraction of commercial gyms.",
    offer: "€0–30/semester",
    url: "https://www.studis-online.de/Studieren/studi-apps.php",
    value: "Save 100% vs. gym",
  },
  {
    id: "smb",
    name: "SMB Museums Berlin",
    category: "culture",
    pricing: "paid",
    tagline: "50% off Berlin museum island",
    description:
      "Pergamon, Neues Museum, Alte Nationalgalerie — students pay €7–8 instead of €14–16 at Berlin's State Museums.",
    offer: "€7–8 (was €14–16)",
    url: "https://www.smb.museum",
    value: "Save 50%",
  },
  {
    id: "philharmoniker",
    name: "Berliner Philharmoniker (Standby)",
    category: "culture",
    pricing: "paid",
    tagline: "Standby tickets for €15",
    description:
      "World-class orchestra, budget price: standby tickets for students cost €15 instead of the regular €50–100. Worth the queue.",
    offer: "€15 standby (was €50–100)",
    url: "https://www.berliner-philharmoniker.de",
    value: "Save 70–85%",
  },
  {
    id: "deutschestheater",
    name: "Deutsches Theater Berlin",
    category: "culture",
    pricing: "paid",
    tagline: "50% off theater tickets",
    description:
      "Students see premieres and classics at Berlin's Deutsches Theater for €10–15 instead of €20–40.",
    offer: "€10–15 (was €20–40)",
    url: "https://www.deutschestheater.de",
    value: "Save 50%",
  },
  {
    id: "yorck",
    name: "Yorck Kinos Berlin",
    category: "culture",
    pricing: "paid",
    tagline: "Cinema Tuesdays/Wednesdays for less",
    description:
      "Arthouse and mainstream cinema at Babylon Mitte and Yorck venues: student tickets €6–8 on Tuesdays and Wednesdays instead of €12–14.",
    offer: "€6–8 (was €12–14)",
    url: "https://www.yorck.de",
    value: "Save 40–50%",
  },
  {
    id: "zooberlin",
    name: "Zoo Berlin",
    category: "culture",
    pricing: "paid",
    tagline: "Student day ticket €13.50",
    description:
      "Germany's oldest and most species-rich zoo offers students entry at €13.50 instead of €17 — a cheap study break in the Tiergarten.",
    offer: "€13.50 (was €17)",
    url: "https://www.zoo-berlin.de",
    value: "Save 21%",
  },
  {
    id: "surface",
    name: "Microsoft Surface (Student)",
    category: "hardware",
    pricing: "paid",
    tagline: "5–10% off Surface devices",
    description:
      "Surface laptops, tablets, and accessories at student pricing via Microsoft's education store — typically 5–10% off.",
    offer: "5–10% off",
    url: "https://www.microsoft.com/de-de/store/b/education",
    value: "Save ~10%",
  },
  {
    id: "macbook",
    name: "Apple MacBook (Education Store)",
    category: "hardware",
    pricing: "paid",
    tagline: "5–9% off + gift card",
    description:
      "MacBook Air and Pro at education pricing with occasional gift cards up to €120 — the standard setup for design and CS students.",
    offer: "5–9% off",
    url: "https://www.apple.com/de-edu/shop",
    value: "Save up to €120 extra",
  },
  {
    id: "ipad",
    name: "Apple iPad (Education Store)",
    category: "hardware",
    pricing: "paid",
    tagline: "5–7% off + gift card",
    description:
      "iPads at education pricing, often with gift cards up to €80 during promo season — great for handwritten notes with Apple Pencil.",
    offer: "5–7% off",
    url: "https://www.apple.com/de-edu/shop",
    value: "Save up to €80 extra",
  },
  {
    id: "applecare",
    name: "AppleCare+ (Student)",
    category: "hardware",
    pricing: "paid",
    tagline: "10% off device protection",
    description:
      "Extend coverage for your education-store Mac or iPad with 10% off AppleCare+ — cheap insurance for an expensive device.",
    offer: "10% off",
    url: "https://www.apple.com/de-edu/shop",
    value: "Save 10%",
  },
  {
    id: "codex",
    name: "OpenAI Codex Student Perk",
    category: "development",
    pricing: "free",
    tagline: "2,500 extra Codex credits",
    description:
      "A one-time reserve of 2,500 Codex credits that kicks in after your regular weekly allowance runs out. Verify your enrollment status, then claim it in the Codex student perks area.",
    offer: "Free credit top-up",
    url: "https://openai.com/codex/",
    value: "2,500 extra credits",
  },
  {
    id: "kiro",
    name: "Kiro for Students",
    category: "development",
    pricing: "free",
    tagline: "AI-assisted development program",
    description:
      "Kiro's student program for AI-powered software development. Check whether your university is on the eligible institutions list before signing up.",
    offer: "Depends on university",
    url: "https://kiro.dev/students",
    value: "Varies by program",
  },
  {
    id: "gitkraken",
    name: "GitKraken",
    category: "development",
    pricing: "freemium",
    tagline: "Git GUI with merge-conflict editor",
    description:
      "Visual Git client with an intuitive interface and a powerful merge-conflict editor. Students get 6 months free via the GitHub Student Pack, then 80% off.",
    offer: "6 months free, then 80% off",
    url: "https://www.gitkraken.com",
    value: "Save $48/year",
  },
  {
    id: "termius",
    name: "Termius",
    category: "development",
    pricing: "freemium",
    tagline: "SSH client for desktop and mobile",
    description:
      "Modern SSH client with sync across desktop and mobile devices. The student program grants free Pro features while enrolled.",
    offer: "Free or Pro features for students",
    url: "https://termius.com/education",
    value: "$10/month",
  },
  {
    id: "bootstrapstudio",
    name: "Bootstrap Studio",
    category: "design",
    pricing: "free",
    tagline: "Build responsive sites with Bootstrap",
    description:
      "Desktop app for creating responsive websites with the Bootstrap framework, with drag-and-drop components and clean code export. Free for students via the GitHub Student Pack.",
    offer: "Free license",
    url: "https://bootstrapstudio.io",
    value: "$29 one-time",
  },
  {
    id: "browserstack",
    name: "BrowserStack",
    category: "development",
    pricing: "free",
    tagline: "Test on real browsers and devices",
    description:
      "Cross-browser testing on 3,000+ real devices and browsers in the cloud. Free for students for one year via the GitHub Student Pack.",
    offer: "Free for 1 year",
    url: "https://www.browserstack.com",
    value: "$1,500/year",
  },
  {
    id: "localstack",
    name: "LocalStack",
    category: "cloud",
    pricing: "free",
    tagline: "Local AWS emulator",
    description:
      "Run AWS services locally for development and testing without touching the real cloud. Students get a free license via the GitHub Student Pack.",
    offer: "Free license",
    url: "https://localstack.cloud",
    value: "Varies by tier",
  },
  {
    id: "polypane",
    name: "Polypane",
    category: "development",
    pricing: "free",
    tagline: "Browser for responsive web development",
    description:
      "A browser built for developers that renders your site in multiple viewports at once, with accessibility and debugging tools. Free for one year for students.",
    offer: "Free for 1 year",
    url: "https://polypane.app",
    value: "$20/month",
  },
  {
    id: "travisci",
    name: "Travis CI",
    category: "development",
    pricing: "free",
    tagline: "Continuous integration",
    description:
      "CI service that automatically builds and tests your code on every push. Students get free private builds via the GitHub Student Pack.",
    offer: "Free private builds",
    url: "https://travis-ci.com",
    value: "$69/month",
  },
  {
    id: "requestly",
    name: "Requestly",
    category: "development",
    pricing: "free",
    tagline: "Intercept and mock HTTP requests",
    description:
      "Debug, modify, and mock network requests in the browser. Students get one year of the Professional plan free via the GitHub Student Pack.",
    offer: "1 year Professional free",
    url: "https://requestly.io",
    value: "$270/year",
  },
  {
    id: "doppler",
    name: "Doppler",
    category: "security",
    pricing: "free",
    tagline: "Secrets management",
    description:
      "Central, secure storage for environment variables and API secrets with sync to your stack. The Team plan is free for students via the GitHub Student Pack.",
    offer: "Free Team plan",
    url: "https://www.doppler.com",
    value: "$8/user/month",
  },
  {
    id: "configcat",
    name: "ConfigCat",
    category: "development",
    pricing: "free",
    tagline: "Feature flag service",
    description:
      "Release features safely with feature flags, A/B testing, and targeting rules. Students get 1,000 free feature flags via the GitHub Student Pack.",
    offer: "1,000 feature flags free",
    url: "https://configcat.com",
    value: "$99/month",
  },
  {
    id: "devcycle",
    name: "DevCycle",
    category: "development",
    pricing: "free",
    tagline: "Feature flag platform",
    description:
      "Open-source feature management platform for shipping behind flags and rolling back instantly. One year of the Starter plan is free for students.",
    offer: "1 year Starter free",
    url: "https://devcycle.com",
    value: "$150+/year",
  },
  {
    id: "workingcopy",
    name: "Working Copy",
    category: "development",
    pricing: "free",
    tagline: "Git client for iOS and iPadOS",
    description:
      "Full-featured Git client for iPhone and iPad with cloning, editing, committing, and pushing — even from an iPad. The Pro upgrade is free for students.",
    offer: "Pro free",
    url: "https://workingcopy.app",
    value: "$24.99 one-time",
  },
  {
    id: "tower",
    name: "Tower",
    category: "development",
    pricing: "free",
    tagline: "Git client for macOS and Windows",
    description:
      "Powerful, polished Git desktop client with drag-and-drop actions, conflict wizards, and partial commits. Pro is free for students via the GitHub Student Pack.",
    offer: "Pro free",
    url: "https://www.git-tower.com",
    value: "$99/year",
  },
  {
    id: "sqlgate",
    name: "SQLGate",
    category: "development",
    pricing: "free",
    tagline: "SQL IDE for multiple databases",
    description:
      "Lightweight database IDE supporting Oracle, MySQL, PostgreSQL, MariaDB, SQL Server, and more. One year free for students.",
    offer: "Free for 1 year",
    url: "https://www.sqlgate.com",
    value: "$119/year",
  },
  {
    id: "popsql",
    name: "PopSQL",
    category: "development",
    pricing: "free",
    tagline: "Collaborative SQL editor",
    description:
      "Modern SQL editor with shared queries, dashboards, and real-time collaboration. The Premium plan is free for students via the GitHub Student Pack.",
    offer: "Premium free",
    url: "https://popsql.com",
    value: "$40/user/month",
  },
  {
    id: "vaadin",
    name: "Vaadin Pro",
    category: "development",
    pricing: "free",
    tagline: "Java web framework",
    description:
      "Full-stack Java framework with a rich set of Pro UI components and tools. The Pro access is free for students via the GitHub Student Pack.",
    offer: "Pro access free",
    url: "https://vaadin.com",
    value: "$1,400/year",
  },
  {
    id: "xojo",
    name: "Xojo Pro",
    category: "development",
    pricing: "free",
    tagline: "Cross-platform development",
    description:
      "Build native apps for desktop, web, iOS, and Android with one language. The Pro license is free for students via the GitHub Student Pack.",
    offer: "Pro license free",
    url: "https://www.xojo.com",
    value: "$299/year",
  },
  {
    id: "blockchair",
    name: "Blockchair",
    category: "development",
    pricing: "free",
    tagline: "Blockchain data and APIs",
    description:
      "Blockchain explorer and API platform covering 40+ chains. Students get 100,000 free API requests via the GitHub Student Pack.",
    offer: "100k API requests free",
    url: "https://blockchair.com",
    value: "$99/month",
  },
  {
    id: "scrimba",
    name: "Scrimba",
    category: "learning",
    pricing: "free",
    tagline: "Interactive coding courses",
    description:
      "Learn JavaScript, React, and Python with interactive screencasts you can edit live. One month of Pro is free for students via the GitHub Student Pack.",
    offer: "1 month Pro free",
    url: "https://scrimba.com",
    value: "$20",
  },
  {
    id: "educative",
    name: "Educative",
    category: "learning",
    pricing: "freemium",
    tagline: "Interactive programming and ML courses",
    description:
      "Text-based interactive courses on programming, system design, and machine learning. Students get 6 months free plus 30% off via the GitHub Student Pack.",
    offer: "6 months free + 30% off",
    url: "https://www.educative.io",
    value: "$120",
  },
  {
    id: "symfonycasts",
    name: "SymfonyCasts",
    category: "learning",
    pricing: "free",
    tagline: "Symfony and PHP courses",
    description:
      "In-depth video tutorials and courses on Symfony, PHP, and JavaScript. Three months free for students via the GitHub Student Pack.",
    offer: "3 months free",
    url: "https://symfonycasts.com",
    value: "$75",
  },
  {
    id: "gorails",
    name: "GoRails",
    category: "learning",
    pricing: "free",
    tagline: "Ruby, Rails and JavaScript tutorials",
    description:
      "Video tutorials on Ruby on Rails, Ruby, and JavaScript for real-world web development. 12 months free for students via the GitHub Student Pack.",
    offer: "12 months free",
    url: "https://gorails.com",
    value: "$228/year",
  },
  {
    id: "bootdev",
    name: "Boot.dev",
    category: "learning",
    pricing: "free",
    tagline: "Backend, DevOps and data courses",
    description:
      "Project-based courses on backend development, DevOps, and data analysis. Three months free for students via the GitHub Student Pack.",
    offer: "3 months free",
    url: "https://boot.dev",
    value: "$120",
  },
  {
    id: "codedex",
    name: "Codedex",
    category: "learning",
    pricing: "free",
    tagline: "Python, web dev and React courses",
    description:
      "Gamified, interactive coding courses for beginners. Six months of the Club plan is free for students via the GitHub Student Pack.",
    offer: "6 months Club free",
    url: "https://www.codedex.io",
    value: "$48",
  },
  {
    id: "khanmigo",
    name: "Khanmigo",
    category: "ai",
    pricing: "freemium",
    tagline: "AI tutor for learning content",
    description:
      "Khan Academy's AI tutor that guides through exercises step by step instead of just giving answers. Free for teachers; students pay a small monthly fee.",
    offer: "Free for teachers / ~$4/month students",
    url: "https://www.khanacademy.org/khan-labs",
    value: "$4/month",
  },
  {
    id: "socratic",
    name: "Socratic",
    category: "learning",
    pricing: "free",
    tagline: "Explain homework with a photo",
    description:
      "Google's learning app that explains homework tasks step by step — just photograph the problem. Completely free.",
    offer: "Free",
    url: "https://socratic.org",
    value: "$0",
  },
  {
    id: "notebooklm",
    name: "NotebookLM",
    category: "ai",
    pricing: "free",
    tagline: "AI grounded in your own sources",
    description:
      "Google's research assistant that analyzes your own documents and answers with source references — ideal for literature research and exam prep. Free.",
    offer: "Free",
    url: "https://notebooklm.google.com",
    value: "$0",
  },
  {
    id: "elicit",
    name: "Elicit",
    category: "ai",
    pricing: "freemium",
    tagline: "Literature and paper research with AI",
    description:
      "AI research assistant that finds and summarizes academic papers and extracts key findings. Free tier with limits for students.",
    offer: "Free with limits",
    url: "https://elicit.org",
    value: "$0",
  },
  {
    id: "andisearch",
    name: "Andi Search",
    category: "ai",
    pricing: "free",
    tagline: "AI search with sources",
    description:
      "Ad-free AI-powered search engine that answers questions with cited sources instead of a link list. Free.",
    offer: "Free",
    url: "https://andisearch.com",
    value: "$0",
  },
  {
    id: "chatgptplus",
    name: "ChatGPT Plus",
    category: "ai",
    pricing: "freemium",
    tagline: "Advanced AI assistant",
    description:
      "OpenAI's assistant with the latest models, image generation, and file analysis. Occasional student promos with free access — check the current offer.",
    offer: "Occasional promo, possibly free",
    url: "https://chatgpt.com",
    value: "$20/month",
  },
  {
    id: "claudefree",
    name: "Claude Free",
    category: "ai",
    pricing: "free",
    tagline: "Explanations, projects and AI chat",
    description:
      "Anthropic's AI assistant with generous free usage — great for explanations, summaries, and working through your own documents and projects.",
    offer: "Free",
    url: "https://claude.ai",
    value: "$0",
  },
  {
    id: "geminiadvanced",
    name: "Google Gemini Advanced",
    category: "ai",
    pricing: "freemium",
    tagline: "AI assistant, docs analysis and storage bundle",
    description:
      "Google's most capable AI models plus 2 TB of storage in the One AI Premium bundle. 12 months free for students, region-limited — check availability.",
    offer: "12 months free (region-limited)",
    url: "https://one.google.com/education",
    value: "$20/month",
  },
  {
    id: "1password",
    name: "1Password",
    category: "security",
    pricing: "free",
    tagline: "Password manager",
    description:
      "One of the best password managers for storing logins, passkeys, and documents securely. Free for students for one year via the GitHub Student Pack.",
    offer: "Free for 1 year",
    url: "https://1password.com",
    value: "$36/year",
  },
  {
    id: "bitwarden",
    name: "Bitwarden",
    category: "security",
    pricing: "free",
    tagline: "Password manager with sync",
    description:
      "Open-source password manager with free sync across all devices — a solid, free alternative for managing all your student accounts.",
    offer: "Free",
    url: "https://bitwarden.com",
    value: "$10/year",
  },
  {
    id: "dashlane",
    name: "Dashlane",
    category: "security",
    pricing: "freemium",
    tagline: "Password manager",
    description:
      "Password manager with VPN and dark-web monitoring. Students get 6 months of the Premium plan free via the GitHub Student Pack.",
    offer: "6 months Premium free",
    url: "https://www.dashlane.com",
    value: "$40",
  },
  {
    id: "opensourcedevcert",
    name: "Let's Encrypt",
    category: "security",
    pricing: "free",
    tagline: "Free TLS certificates for everyone",
    description:
      "Free, automated TLS certificates for any website — the standard for HTTPS on student projects and portfolios. In addition to Namecheap's free first-year SSL via the GitHub Student Pack.",
    offer: "Free",
    url: "https://letsencrypt.org/",
    value: "$0",
  },
  {
    id: "astra",
    name: "Astra Security",
    category: "security",
    pricing: "free",
    tagline: "Website firewall and malware scanner",
    description:
      "Security suite for websites: firewall, malware scanner, and pentest. Six months free for students via the GitHub Student Pack.",
    offer: "6 months free",
    url: "https://www.getastra.com",
    value: "$33/month",
  },
  {
    id: "newrelic",
    name: "New Relic",
    category: "cloud",
    pricing: "free",
    tagline: "Observability and performance monitoring",
    description:
      "Full-stack observability: monitor servers, apps, and infrastructure in one platform. The free student account includes a substantial monthly data allowance.",
    offer: "Free account",
    url: "https://newrelic.com/signup",
    value: "$300/month worth",
  },
  {
    id: "datadog",
    name: "Datadog",
    category: "cloud",
    pricing: "free",
    tagline: "Monitoring for servers and applications",
    description:
      "Cloud-scale monitoring and observability for infrastructure, logs, and traces. Students get the Pro account free for 2 years via the GitHub Student Pack.",
    offer: "Pro free for 2 years",
    url: "https://www.datadoghq.com",
    value: "$300/month",
  },
  {
    id: "honeybadger",
    name: "Honeybadger",
    category: "cloud",
    pricing: "free",
    tagline: "Exception and uptime monitoring",
    description:
      "Error tracking and uptime monitoring focused on developer experience. The Small plan is free for one year for students.",
    offer: "Small plan free (1 year)",
    url: "https://www.honeybadger.io",
    value: "$299/year",
  },
  {
    id: "blackfire",
    name: "Blackfire",
    category: "cloud",
    pricing: "free",
    tagline: "Code performance analysis",
    description:
      "Profiling and performance monitoring for PHP, Python, and Go applications. The Developer plan is free for students via the GitHub Student Pack.",
    offer: "Developer plan free",
    url: "https://www.blackfire.io",
    value: "$109/year",
  },
  {
    id: "sentry",
    name: "Sentry",
    category: "cloud",
    pricing: "freemium",
    tagline: "Error tracking",
    description:
      "Track exceptions and performance issues across your whole stack in real time. Students get the Team plan free for one year via the GitHub Student Pack.",
    offer: "Team features free (1 year)",
    url: "https://sentry.io",
    value: "$26/month",
  },
  {
    id: "deepnote",
    name: "Deepnote",
    category: "cloud",
    pricing: "free",
    tagline: "Collaborative data notebooks",
    description:
      "Real-time collaborative notebooks for data science, compatible with Jupyter. The Team plan is free for students via the GitHub Student Pack.",
    offer: "Team plan free",
    url: "https://deepnote.com",
    value: "$40/user/month",
  },
  {
    id: "camber",
    name: "Camber",
    category: "cloud",
    pricing: "free",
    tagline: "AI-powered data science compute",
    description:
      "Cloud compute resources for data science workloads with AI assistance. The Student plan is free via the GitHub Student Pack.",
    offer: "Student plan free",
    url: "https://camber.ai",
    value: "$40/month",
  },
  {
    id: "appwrite",
    name: "Appwrite",
    category: "cloud",
    pricing: "freemium",
    tagline: "Backend infrastructure for apps",
    description:
      "Open-source backend-as-a-service: auth, databases, storage, and functions. The Education plan is free for students via the GitHub Student Pack.",
    offer: "Education plan free",
    url: "https://appwrite.io",
    value: "$40/month",
  },
  {
    id: "gettech",
    name: "get.tech Domain",
    category: "productivity",
    pricing: "free",
    tagline: "Free .TECH domain for a year",
    description:
      "Register a free .TECH domain for one year — perfect for tech projects and portfolios. Via the GitHub Student Pack.",
    offer: "1 year .TECH domain free",
    url: "https://get.tech",
    value: "$50/year",
  },
  {
    id: "githubpages",
    name: "GitHub Pages",
    category: "productivity",
    pricing: "free",
    tagline: "Static hosting from GitHub repos",
    description:
      "Host static websites directly from a GitHub repository, free with a custom domain support — the easiest way to put a portfolio online.",
    offer: "Free",
    url: "https://pages.github.com",
    value: "$0",
  },
  {
    id: "pageclip",
    name: "Pageclip",
    category: "productivity",
    pricing: "free",
    tagline: "Backend for static site forms",
    description:
      "Collect form submissions from static websites without writing any server code. The Basic plan is free for students via the GitHub Student Pack.",
    offer: "Basic plan free",
    url: "https://pageclip.co",
    value: "$8/month",
  },
  {
    id: "pomodone",
    name: "PomoDone App",
    category: "productivity",
    pricing: "free",
    tagline: "Pomodoro timer and task integration",
    description:
      "Pomodoro timer that integrates with your existing task tools (Todoist, Trello, and more). Two years of the Lite plan are free for students.",
    offer: "2 years Lite free",
    url: "https://pomodoneapp.com",
    value: "$58",
  },
  {
    id: "hazeover",
    name: "HazeOver",
    category: "productivity",
    pricing: "free",
    tagline: "Focus app for macOS",
    description:
      "Dims background windows so you can focus on the one you're working in — great for deep study sessions. Free license for students.",
    offer: "Free license",
    url: "https://hazeover.com",
    value: "$4.99 one-time",
  },
  {
    id: "todiagram",
    name: "ToDiagram",
    category: "productivity",
    pricing: "free",
    tagline: "Turn JSON, YAML and CSV into diagrams",
    description:
      "Visualize and edit JSON, YAML, CSV, and more as interactive diagrams. The Pro plan is free for students via the GitHub Student Pack.",
    offer: "Pro free",
    url: "https://todiagram.com",
    value: "$10/month",
  },
  {
    id: "poeditor",
    name: "POEditor",
    category: "productivity",
    pricing: "free",
    tagline: "Localization management",
    description:
      "Manage translations and localization files for apps and websites with your team. One year of the Plus plan is free for students.",
    offer: "1 year Plus free",
    url: "https://poeditor.com",
    value: "$174/year",
  },
  {
    id: "themeisle",
    name: "ThemeIsle",
    category: "design",
    pricing: "free",
    tagline: "WordPress themes",
    description:
      "Premium WordPress themes and plugins (Neve, Hestia, and more). One year free for students via the GitHub Student Pack.",
    offer: "1 year free",
    url: "https://themeisle.com",
    value: "$99/year",
  },
  {
    id: "icons8",
    name: "Icons8",
    category: "design",
    pricing: "free",
    tagline: "Icons, photos and illustrations",
    description:
      "Huge library of icons, photos, and illustrations in consistent styles. Three months free for students via the GitHub Student Pack.",
    offer: "3 months free",
    url: "https://icons8.com",
    value: "$30",
  },
  {
    id: "iconscout",
    name: "IconScout",
    category: "design",
    pricing: "free",
    tagline: "Icons, 3D assets and animations",
    description:
      "Millions of icons, illustrations, 3D assets, and Lottie animations. 60 premium icons per month free for one year for students.",
    offer: "60 premium icons/month (1 year)",
    url: "https://iconscout.com",
    value: "$120/year",
  },
  {
    id: "visme",
    name: "Visme",
    category: "design",
    pricing: "free",
    tagline: "Presentations and visual documents",
    description:
      "Design tool for presentations, infographics, and reports with templates and charts. Three months free for students via the GitHub Student Pack.",
    offer: "3 months free",
    url: "https://www.visme.co",
    value: "$59",
  },
  {
    id: "simpleanalytics",
    name: "Simple Analytics",
    category: "productivity",
    pricing: "free",
    tagline: "Privacy-friendly web analytics",
    description:
      "GDPR-friendly analytics without cookies or tracking your visitors across sites. One year of the Starter plan is free for students.",
    offer: "1 year Starter free",
    url: "https://simpleanalytics.com",
    value: "$228/year",
  },
  {
    id: "appfigures",
    name: "Appfigures",
    category: "productivity",
    pricing: "free",
    tagline: "App store analytics",
    description:
      "Track downloads, revenue, and rankings for apps across all stores in one dashboard. One year free for students via the GitHub Student Pack.",
    offer: "1 year free",
    url: "https://appfigures.com",
    value: "$120/year",
  },
];
